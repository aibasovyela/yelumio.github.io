...
const modules = [{
  ...
  icon: FileText,
  title: "ТЕКСТ",
  points: ["Как писать сценарии и хуки", "Как текст управляет визуалом", "Ошибки в формулировках"]
}, {
  icon: Camera,
  title: "ФОТО",
...
const mistakes = ["Длинные промпты", "Хаотичное движение", "Красивая картинка без смысла", "Игнор звука", "Ожидание «волшебной кнопки»"];
export const Module0Section = () => {
  return <section className="section-padding relative overflow-hidden py-0 pt-0">
      ...
      <div className="container relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {modules.map((module, index) => <ScrollReveal key={index} delay={index * 100}>
              <GlassCard className="space-y-4 h-full">
                ...
                <ul className="space-y-2">
                  {module.points.map((point, i) => <li key={i} className="flex items-start gap-2 text-foreground/80">
          ...
          <GlassCard className="md:col-span-2 lg:col-span-3 lg:max-w-3xl lg:mx-auto bg-destructive-foreground border-destructive/20">
            ...
            <div className="flex flex-wrap gap-3">
              {mistakes.map((mistake, index) => <span key={index} className="px-4 py-2 rounded-full bg-foreground/5 text-sm font-medium border border-foreground/10">
                  {mistake}
                </span>)}
            </div>
          </GlassCard>
